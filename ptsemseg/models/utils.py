...
class unetConv2(nn.Module):
    ...
    def forward(self, inputs):
        ...
        return outputs
############################  ABN
class unetConv2ABN(nn.Module):
    def __init__(self, in_size, out_size, is_batchnorm):
        super(unetConv2ABN, self).__init__()

        if is_batchnorm:
            self.conv1 = nn.Sequential(
                nn.Conv2d(in_size, out_size, 3, 1, 1),
                InPlaceABN(out_size, activation="relu")
            )
            self.conv2 = nn.Sequential(
                nn.Conv2d(out_size, out_size, 3, 1, 1),
                InPlaceABN(out_size, activation="relu")
...
class unetUpABN(nn.Module):
    def __init__(self, in_size, out_size, is_deconv):
        super(unetUpABN, self).__init__()
        self.conv = unetConv2ABN(in_size, out_size, False)
        if is_deconv:
            self.up = nn.ConvTranspose2d(in_size, out_size, kernel_size=2, stride=2)
        else:
            self.up = nn.UpsamplingBilinear2d(scale_factor=2)
...
#ME: note: this upsamples one input, pads the other to make the H, W dims fit, and then concats
class myunetUp(nn.Module):
    ...
    def forward(self, inputs1, inputs2):
        #print("1st and 2nd arg size: ", inputs1.size(),inputs2.size())
        outputs2 = self.up(inputs2)
        #offset = outputs2.size()[2] - inputs1.size()[2]  # square assumption? #original
        offset_height = outputs2.size()[2] - inputs1.size()[2]
        offset_width = outputs2.size()[3] - inputs1.size()[3]
        #print(offset_height, offset_width)
        #print(offset2)   # looks like the problem is that the offset for H,W differ
        #if offset != offset2:
        #    if abs(offset) > abs(offset2):
        #        offset2 = offset
        #    else:
        #        offset = offset2
        #offset = offset2
        #print(offset)
        #padding = 2 * [offset // 2, offset // 2] # original
        padding = [math.floor(offset_width / 2), math.ceil(offset_width / 2), math.floor(offset_height / 2), math.ceil(offset_height / 2)]
        #padding = [-4,-4,-5,-4]
        #print(padding)
        #padding = [-4,-4,-4,-4]
        #padding = [offset // 2, offset // 2, (offset // 2)-1, (offset // 2)]  # this didn't help. NEED TO UNDERSTAND F.pad
        outputs1 = F.pad(inputs1, padding)
        #print("size of the padded concat tensor and upsampled tensor respectively before concat", outputs1.size(), outputs2.size())
        return self.conv(torch.cat([outputs1, outputs2], 1))
    ...
    def __init__(
        ...
        for i in range(len(pool_sizes)):
            self.paths.append(
                conv2DBatchNormRelu(
                    in_channels,
                    int(in_channels / len(pool_sizes)),  # set out channels
                    1,
                    1,
                    0,
                    bias=bias,
